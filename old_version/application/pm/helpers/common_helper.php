...
function decodeAuthors ($author){
  $authors = json_decode($author, true);
  $authorString = "";

  if(sizeof($authors) == 1) {
    $authorString = $authors[0]['name']['given-names']." ".$authors[0]['name']['surname'];  
  }
  else if (sizeof($authors) == 2) {
    $authorString = $authors[0]['name']['given-names']." ".$authors[0]['name']['surname'];  
    $authorString .= " and " .$authors[1]['name']['given-names']." ".$authors[1]['name']['surname'];  
  }
  else if (sizeof($authors) > 2) {
    $authorString = $authors[0]['name']['given-names']." ".$authors[0]['name']['surname'];  
...
function buildEditableAlertTabs($alert_array, $journals, $output_path) {
  ...
  foreach($all_ads_by_journal as $journal => $lines) {
    ...
    foreach ($lines as $line) {
      ...
      array_push($checkboxes, $CI->load->view('shared/checkbox', array('label' => $label,'id' => $input_element_id,), TRUE ) );
