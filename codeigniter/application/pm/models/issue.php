...
class Issue extends CI_Model {
 ...
 //$this->section = $p['section'];
    ...
    $this->cover_caption = $p['cover_caption'];
   ...
   * get_publish_date
   *
   * @param mixed string in date() format 
   *   ex.  $issue->get_publish_date('d M Y'); will supply, e.g., 24 Apr 2013
   * @access public
   * @return void
   */
  function get_publish_date($format = null) {
    if (empty($format)) {
  ...
  function get_issue_cover() {
    return $this->path_strtr($this->issue_cover);
  }
  function get_cover_caption() {
    $caption_path = $this->path_strtr($this->xml_path . $this->cover_caption);
    if (!empty($this->_cover_caption_text)) { 
      return $this->_cover_caption_text; 
    } 
    if (file_exists($caption_path)) {
      $doc = new DOMDocument("1.0", "UTF-8");
      @$doc->loadHTMLFile($caption_path); // silence malformed HTML
      $xpath = new DOMXPath($doc);
      $caption = $xpath->query("//body")->item(0)->nodeValue;
      $this->_cover_caption_text = $caption; 
      $this->_cover_caption_html = $doc->saveHTML($xpath->query("//body")->item(0));
      return $caption;
    };
    return false;
  }
  ...
  function get_cover_caption_asHTML() {
    if (!empty($this->_cover_caption_html)) {
      return $this->_cover_caption_html; 
    } else {
      $this->get_cover_caption();
      return $this->_cover_caption_html;
    }
  }
  ...
  function get_xml_path($name) {
    ...
    if (is_null($path)) { 
      show_error('Path for \'data_source\' ' . $ds . ' not found in pm_config');
    } 
    return $this->path_strtr($this->xml_path . $path);
  }
  /**
   * get_articles
   *
   ...
   *   The default is 'Articles' the only exception at this time
   *   is ScienceXpress in the Science publication
   * @access public
   * @return array of Article
   */
  function get_articles($name = 'Articles') {
    $this->data_source = $name;
    $articles = array();
    if ($article_list = $this->get_article_list($name)) {
      foreach ($article_list as $article) {
        $articles[] = $this->new_article($article);
      }
      return $articles;
    } else {
      throw new Exception("Could not get articles from data source: {$name}");
    }
  }
  /**
   * get_article_list
  ...
  function get_article_list($name) {
    try {
      ...
      error_log("getDirecetoryList error: " . $e->getMessage() . "\n");
      return false;
    }
    $finfo = finfo_open();
    $xml_files = array();
  ...
  function new_article($file) {
    ...
    // Ugly hack for Science Express. A  real fix will involve
    // several controllers, pm_config, and sci_alert_config
    // and maybe common_helper. Pushing off for now
    if ($this->data_source === 'ScienceXpress') {
      $A->set_base_url("http://www.sciencemag.org/lookup/doi/DOI");
    } else {
  ...
  function filter_articles(&$arr_array, $el, $pattern) {
    ...
    foreach ($arr_array as $article) {
      ...
      // uses $el to build getters to run against the article
